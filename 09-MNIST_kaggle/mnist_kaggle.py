...
Y_train = train["label"]  # train labels
...
Y_train = to_categorical(Y_train, num_classes = 10)  # Encode labels to one hot vectors
random_seed = 2  # Set the random seed
X_train, X_val, Y_train, Y_val = train_test_split(X_train, Y_train, test_size = 0.1, random_state=random_seed)

# Define the model: In -> [[Conv2D->relu]*2 -> MaxPool2D -> Dropout]*2 -> Flatten -> Dense -> Dropout -> Out
model = Sequential()

model.add(Conv2D(filters = 32, kernel_size = (5,5),padding = 'Same', activation ='relu', input_shape = (28,28,1)))  # (?,28,28,32)
model.add(Conv2D(filters = 32, kernel_size = (5,5),padding = 'Same', activation ='relu'))  # (28,28,32)
model.add(MaxPool2D(pool_size=(2,2)))  # (?,14,14,32)
model.add(Dropout(0.25))  # (?,14,14,32)

model.add(Conv2D(filters = 64, kernel_size = (3,3),padding = 'Same', activation ='relu'))  # (?,14,14,64)
model.add(Conv2D(filters = 64, kernel_size = (3,3),padding = 'Same', activation ='relu'))  # (?,14,14,64)
...
learning_rate_reduction = ReduceLROnPlateau(monitor='val_acc', patience=3, verbose=1, factor=0.5, min_lr=0.00001)  # 当评价指标不再提升时,减少学习率
# 下面两组参数是提交到kaggle结果较好的参数
...
# Look at confusion matrix. Not supported by the server.
...
true_prob_errors = np.diagonal(np.take(Y_pred_errors, Y_true_errors, axis=1)) #Predicted probabilities of true values in error set
delta_pred_true_errors = Y_pred_errors_prob - true_prob_errors #Difference between probability of predicted label and the true label
...
# Result
results = model.predict(test)  # predict results
results = np.argmax(results, axis = 1)  # select the indix with the maximum probability
